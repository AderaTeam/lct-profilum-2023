...
export class AnalyzeService {
    ...
    public async analyzeUserById(userid: number)
    {
        ...
        if(userSocials.filter((obj) => obj.social.name == 'VK'))
        {
            ...
            var vkResultWorks = (await axios.get(`https://178.170.192.87:9000/vk/simple_analize_interests/?user_id=${userVKid}&n_of_works=3`)).data
        }  
        if(userSocials.filter((obj) => obj.social.name == 'LeaderID'))
        {
            ...
            var leaderResultWorks = (await axios.get(`https://178.170.192.87:9000/leaderid/get_works/?user_id=${leaderID}&n_of_works=3`)).data
        }
